...
@import "RTCouchImageSearch.j"
@import "RTCouchAnimationSearch.j"
@import "RTCouchFrameSearch.j"

//media types
MKMediaTypeImage    = 1 << 0;
MKMediaTypeVideo    = 1 << 1;

MKMediaTypeSound    = 1 << 2;
MKMediaTypeScript    = 1 << 3;

MKMediaTypeFrame	= 1 << 4;
MKMediaTypeAnimation= 1 << 5;
...
- (id)init
{
    if (self = [super initWithContentRect:CGRectMake(100, 100, 340, 400) styleMask:CPTitledWindowMask|CPResizableWindowMask|CPClosableWindowMask])
    {
        [self setTitle:@"Asset Browser"];
        [self setMinSize:CPSizeMake(340, 300)];
        
        var contentView = [self contentView];
        
        searchField = [CPTextField roundedTextFieldWithStringValue:@"" placeholder:@"Search by tag..." width:256];
        ...
        [filterView setBackgroundColor:[CPColor colorWithRed:220.0/255.0 green:220.0/255.0 blue:240.0/255.0 alpha:1.0]];
		...
		bitmapsRadio = [CPRadio radioWithTitle:@"Images"],
		soundsRadio = [CPRadio radioWithTitle:@"Sounds"],
		scriptsRadio = [CPRadio radioWithTitle:@"Scripts"],
		//videosRadio = [CPRadio radioWithTitle:@"Google Videos"],
		//imagesRadio = [CPRadio radioWithTitle:@"Google Images"],
		//videosRadio = [CPRadio radioWithTitle:@"Google Videos"],
		radioButtons = [allRadio, bitmapsRadio, soundsRadio, scriptsRadio];
...
//        [videosRadio setRadioGroup:searchFilterRadioGroup];
        [bitmapsRadio setRadioGroup:searchFilterRadioGroup];
        [soundsRadio setRadioGroup:searchFilterRadioGroup];
        [scriptsRadio setRadioGroup:searchFilterRadioGroup];
        
        [allRadio setTag:MKMediaTypeAll];
//        [imagesRadio setTag:MKMediaTypeImage];
//        [videosRadio setTag:MKMediaTypeVideo];
		[bitmapsRadio setTag: MKMediaTypeImage];
		[soundsRadio setTag: MKMediaTypeSound];
		[scriptsRadio setTag: MKMediaTypeScript];
...
//        [videosRadio setFrameOrigin:CGPointMake(CGRectGetMaxX([imagesRadio frame]) + 8, CGRectGetMinY([imagesRadio frame]))];
		...
		for (var i=1, count = radioButtons.length; i < count; i++)
        {
			[radioButtons[i] setFrameOrigin: CGPointMake(CGRectGetMaxX([radioButtons[i-1] frame]) + 8, CGRectGetMinY([allRadio frame]))];   
		}
...
//        [videosRadio setAction:@selector(showVideos:)];
      ...
      dataByIdentifier = [CPDictionary dictionary];

        //standard search providers

    [self addSourceWithIdentifier:[RTCouchImageSearch identifier] URL:[RTCouchImageSearch URL] delegate:[RTCouchImageSearch new]];
    [self addSourceWithIdentifier:[RTCouchAnimationSearch identifier] URL:[RTCouchAnimationSearch URL] delegate:[RTCouchAnimationSearch new]];
    [self addSourceWithIdentifier:[RTCouchFrameSearch identifier] URL:[RTCouchFrameSearch URL] delegate:[RTCouchFrameSearch new]];
...
//     [self addSourceWithIdentifier:[MKFlickrSearchDelegate identifier] URL:[MKFlickrSearchDelegate URL] delegate:[MKFlickrSearchDelegate new]];
	...
	// load all images into the asset browser to start
	
    url = [[RTCouchServer sharedCouchServer] serverAndDatabase] + "/_design/assets/_view/images?callback=" + CPJSONPCallbackReplacementString;
	...
	connection.identifier = [RTCouchImageSearch identifier];
    [connectionsByIdentifier setObject:connection forKey: [RTCouchImageSearch identifier] ];
    [connection start];
...
*/
    ...
    return [CPKeyedArchiver archivedDataWithRootObject: content[index]];
}
...
- (void)removeSourceWithIdentifier:(CPString)anIdentifier
{
	...
	[dataByIdentifier removeObjectForKey:anIdentifier];
...
{
	console.log(data);
	[dataByIdentifier setObject: data forKey: aConnection.identifier];
}
...
-(void)connectionDidFinishLoading:(CPURLConnection)aConnection
{
    var identifier = aConnection.identifier,
		...
		data = [dataByIdentifier objectForKey: identifier],
